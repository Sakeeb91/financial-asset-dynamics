...
def simulate_sabr_path(model, t_span, dt, seed=None):
    ...
    for i in range(1, n_steps):
        ...
        F_diffusion = alpha_path[i-1] * (abs(F_path[i-1])**model.beta) * W_F[0, i-1]
        F_path[i] = F_path[i-1] + F_diffusion
        
        # Volatility step (log-normal volatility)
        alpha_diffusion = model.nu * alpha_path[i-1] * W_alpha[0, i-1]
        alpha_path[i] = alpha_path[i-1] + alpha_diffusion
        
        # Ensure positive values (for numerical stability)
        F_path[i] = max(1e-6, F_path[i])
        alpha_path[i] = max(1e-6, alpha_path[i])
    
    return t_points, F_path, alpha_path

def simulate_multiple_sabr_paths(model, t_span, dt, n_paths=10, seed=None):
    """
    ...
    for path_idx in range(n_paths):
        for i in range(1, n_steps):
            # Forward rate step
            F_diffusion = alpha_paths[path_idx, i-1] * (abs(F_paths[path_idx, i-1])**model.beta) * W_F[path_idx, i-1]
            F_paths[path_idx, i] = F_paths[path_idx, i-1] + F_diffusion
            
            # Volatility step
            alpha_diffusion = model.nu * alpha_paths[path_idx, i-1] * W_alpha[path_idx, i-1]
            alpha_paths[path_idx, i] = alpha_paths[path_idx, i-1] + alpha_diffusion
            
            # Ensure positive values
            F_paths[path_idx, i] = max(1e-6, F_paths[path_idx, i])
            alpha_paths[path_idx, i] = max(1e-6, alpha_paths[path_idx, i])
    
    return t_points, F_paths, alpha_paths

def calculate_black_price(F, K, T, sigma, is_call=True):
    """
    Calculate option price using Black's model (for interest rate options).
    ...
    Args:
        ...
        T: Time to expiry
        sigma: Volatility
        is_call: Whether to price a call option (True) or put option (False)
        
    Returns:
        float: Option price as a percentage of notional
    """
    d1 = (np.log(F/K) + 0.5 * sigma**2 * T) / (sigma * np.sqrt(T))
    d2 = d1 - sigma * np.sqrt(T)
    
    if is_call:
        # Cap price (call option on forward rate)
        return F * stats.norm.cdf(d1) - K * stats.norm.cdf(d2)
    else:
        # Floor price (put option on forward rate)
...
if __name__ == "__main__":
    ...
    n_paths = 20
    t_points, forward_paths, alpha_paths = simulate_multiple_sabr_paths(sabr_model, t_span, dt, n_paths, seed=42)
    
    # Calculate mean paths
    mean_forward = np.mean(forward_paths, axis=0)
    mean_alpha = np.mean(alpha_paths, axis=0)
    ...
    for i in range(n_paths):
        ax1.plot(t_points, forward_paths[i] * 100, 'b-', alpha=0.3)  # *100 to show in percentage
    ax1.plot(t_points, mean_forward * 100, 'r-', linewidth=2, label='Mean Path')
    ax1.axhline(f0 * 100, color='k', linestyle=':', label=f'Initial Rate ({f0*100:.1f}%)')
    ax1.set_ylabel('Forward Rate (%)')
    ...
    # Volatility paths
    for i in range(n_paths):
        ax2.plot(t_points, alpha_paths[i], 'r-', alpha=0.3)
    ax2.plot(t_points, mean_alpha, 'b-', linewidth=2, label='Mean Path')
    ...
    # Range of strikes around the forward rate
    strikes = np.linspace(0.01, 0.05, 41)  # 1% to 5%, 41 points
    
    # Different expiries
    expiries = [0.25, 0.5, 1.0, 2.0]  # 3M, 6M, 1Y, 2Y
    
    # Calculate SABR implied volatilities for different strikes and expiries
    plt.figure(figsize=(12, 6))
    
    for T in expiries:
        implied_vols = np.array([sabr_model.sabr_implied_volatility(K, T) for K in strikes])
        plt.plot(strikes * 100, implied_vols, label=f'T = {T:.2f} year{"s" if T > 1 else ""}')
    
    plt.axvline(f0 * 100, color='k', linestyle=':', label=f'Forward Rate ({f0*100:.1f}%)')
    plt.axhline(alpha0, color='k', linestyle='--', label=f'Initial Volatility ({alpha0:.2f})')
    plt.xlabel('Strike Rate (%)')
    plt.ylabel('Implied Volatility')
    plt.title('SABR Model: Volatility Smile/Skew for Different Expiries')
    plt.legend()
    ...
    beta_labels = ['β = 0.0 (Normal)', 'β = 0.5 (CEV)', 'β = 1.0 (Log-normal)']
    
    plt.figure(figsize=(12, 6))
    
    for i, beta_val in enumerate(beta_values):
        # Create model with this beta
        model = SABRModel(alpha, beta_val, nu, rho, f0, alpha0)
        
        # Calculate implied volatilities
        implied_vols = np.array([model.sabr_implied_volatility(K, T) for K in strikes])
        
        # Plot
        plt.plot(strikes * 100, implied_vols, label=beta_labels[i])
    
    plt.axvline(f0 * 100, color='k', linestyle=':', label=f'Forward Rate ({f0*100:.1f}%)')
    plt.xlabel('Strike Rate (%)')
    plt.ylabel('Implied Volatility')
    plt.title('SABR Model: Effect of Beta Parameter on Volatility Smile')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    # Different nu values
    nu_values = [0.2, 0.4, 0.8]
    nu_labels = ['ν = 0.2 (Low)', 'ν = 0.4 (Medium)', 'ν = 0.8 (High)']
    
    plt.figure(figsize=(12, 6))
    
    for i, nu_val in enumerate(nu_values):
        # Create model with this nu
        model = SABRModel(alpha, beta, nu_val, rho, f0, alpha0)
        
        # Calculate implied volatilities
        implied_vols = np.array([model.sabr_implied_volatility(K, T) for K in strikes])
        
        # Plot
        plt.plot(strikes * 100, implied_vols, label=nu_labels[i])
    
    plt.axvline(f0 * 100, color='k', linestyle=':', label=f'Forward Rate ({f0*100:.1f}%)')
    plt.xlabel('Strike Rate (%)')
    plt.ylabel('Implied Volatility')
    plt.title('SABR Model: Effect of Volatility of Volatility Parameter on Volatility Smile')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    rho_values = [-0.8, -0.3, 0.0, 0.3]
    rho_labels = ['ρ = -0.8 (Strong Negative)', 'ρ = -0.3 (Moderate Negative)', 'ρ = 0.0 (No Correlation)', 'ρ = 0.3 (Positive)']
    
    plt.figure(figsize=(12, 6))
    
    for i, rho_val in enumerate(rho_values):
        # Create model with this rho
        model = SABRModel(alpha, beta, nu, rho_val, f0, alpha0)
        
        # Calculate implied volatilities
        implied_vols = np.array([model.sabr_implied_volatility(K, T) for K in strikes])
        
        # Plot
        plt.plot(strikes * 100, implied_vols, label=rho_labels[i])
    
    plt.axvline(f0 * 100, color='k', linestyle=':', label=f'Forward Rate ({f0*100:.1f}%)')
    plt.xlabel('Strike Rate (%)')
    plt.ylabel('Implied Volatility')
    plt.title('SABR Model: Effect of Correlation Parameter on Volatility Smile')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    print(f"{'Strike':^10}|{'SABR Cap':^12}|{'Black Cap':^12}|{'SABR Floor':^12}|{'Black Floor':^12}")
    print("-" * 62)
    
    for i, K in enumerate(option_strikes):
        print(f"{K*100:^10.2f}%|{cap_prices[i]*100:^12.4f}%|{black_cap_prices[i]*100:^12.4f}%|{floor_prices[i]*100:^12.4f}%|{black_floor_prices[i]*100:^12.4f}%")
    
    print("\nSimulation complete! All plots saved to the plots/sabr directory.") 